...
class DataSchemaEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {

  private val system = ActorSystem("KnowledgeEntitySpec", JsonSerializerRegistry.actorSystemSetupFor(KnowledgeRepositorySerializerRegistry))
...
//    val driver = new PersistentEntityTestDriver(system, new SchemaEntity, "knowledge-1")
//    block(driver)
...
//      val cmd = CreateSchema("id", "name", Some("description"), "KNOWLEDGEN", "schema")
//      val outcome = driver.run(cmd)
//      outcome.replies should contain only Schema("id", "name", Some("description"), "KNOWLEDGEN", "schema")
...
//      val cmd = CreateSchema("id", "name", Some("description"), "KNOWLEDGEN", "schema")
//      val cmd2 = UpdateSchema("id", "name1", Some("description1"), "KNOWLEDGEN", "schema")
//      val outcome = driver.run(cmd)
//      outcome.replies should contain only Schema("id", "name", Some("description"), "KNOWLEDGEN", "schema")
//      outcome.events should contain only SchemaCreated(cmd.id, cmd.name, cmd.description, cmd.notation, cmd.schema)
//      val outcome2 = driver.run(cmd2)
//      outcome2.replies should contain only Schema("id", "name1", Some("description1"), "KNOWLEDGEN", "schema")
